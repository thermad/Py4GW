...
from Py4GWCoreLib.modular.recipes import list_available_blocks
...
def _startup_engine_profile() -> tuple[str, str]:
    """
    Pick tester startup profile from currently enabled combat engine.
    Template is always 'preserve' so launching tester never flips HeroAI/auto-combat.
    """
    try:
    ...
    # Keep local tester runtime native-only; backend control steps still
    # broadcast to HeroAI party backends for alts.
    return ("preserve", engine)
...
def _resolve_modular_root() -> str:
    return os.path.normpath(modular_data_root())
...
def _build_empty_selection_phase():
    def _pre_hook(_bot) -> None:
        global _status
        _status = "No block selected. Pick kind/folder/block, then press Start."

    return build_inline_modular_phase(
        display_name="No Modular Block Selected",
        name="Run Selected Modular Block",
        steps=[],
        recipe_name="ModularBlockTest",
        anchor=True,
        pre_run_hook=_pre_hook,
        pre_run_name="No Selection",
    )
...
def _apply_pending_selection_rebuild_if_safe() -> None:
    ...
    try:
        ...
        if selected_key and bot._phases:
            bot._phases[0] = build_modular_block_phase(
                selected_key,
                name="Run Selected Modular Block",
                kind=_get_kind_key(),
                recipe_name="ModularBlockTest",
            ...
            bot._phases[0] = _build_empty_selection_phase()
        loop_to = "Run Selected Modular Block" if _loop_selected_block else None
        bot.set_loop(_loop_selected_block, loop_to=loop_to)
        bot.rebuild_runtime()
        _selection_dirty = False
...
def _draw_main() -> None:
    global _selected_kind_label, _selected_folder, _selected_block, _status, _loop_selected_block, _auto_resize_panel, _debug_logging
    ...
    PyImGui.same_line(0, 12)
    debug_now = PyImGui.checkbox("Debug Logging", _debug_logging)
    if bool(debug_now) != bool(_debug_logging):
        _debug_logging = bool(debug_now)
        bot.set_debug_logging(_debug_logging)
...
def _current_selection_phase():
    ...
    return _build_empty_selection_phase()


bot = ModularBot(
    name="Modular Block Tester",
    phases=[_current_selection_phase()],
    # Loop behavior is applied via native planner runtime rebuilds.
    loop=False,
    ...
    diagnostics_label="Modular Tester",
...
def main():
    ...
    try:
        ...
        ConsoleLog("Modular Tester", f"Runtime error: {exc}", Console.MessageType.Error)
        ConsoleLog("Modular Tester", tb, Console.MessageType.Error)
        try:
            bot.record_diagnostics_event(
                "exception",
                message=f"Tester runtime error: {exc}",
                traceback_text=tb,
            )
        except Exception:
            pass
